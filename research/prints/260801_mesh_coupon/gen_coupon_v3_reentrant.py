...
SHEET_T = 0.8           # 網厚（4 層 @0.2）
PLATE_T = 1.6           # 板總厚

W = 12.0                # 水平格距（垂直桿間距）
HV = 9.0                # 垂直桿長
S = -3.5                # 斜桿垂直位移，負＝內凹＝re-entrant（排距 = HV+S）
ROW_H = HV + S          # 5.5 — 拉伸時斜桿被拉直、橫向同時變寬（負泊松比）
...
sheet_poly = unary_union(ribs + [tabs, plate]).intersection(frame)
...
combined.export("mesh_coupon_v3_reentrant.stl")
vol = combined.volume / 1000.0
print(f"STL out: mesh_coupon_v3_reentrant.stl  triangles={len(combined.faces)}")
...
ax.set_title("mesh coupon v3 re-entrant honeycomb — as printed (already open)")
fig.savefig("preview_v3_reentrant.png", dpi=150, bbox_inches="tight")
print("preview: preview_v3_reentrant.png")
