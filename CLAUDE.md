...
3. **Never commit license-restricted binaries.** The Beatrice engine binary and the trained tenor model stay git-ignored and live outside the repo. Before committing, check `.gitignore` and `git ls-files` — no `.bin/.pth/.onnx`, no `beatrice_engine*`, no `paraphernalia_*`, no model dirs. (The head mesh in `app/assets/` is intentionally allowed — see `app/assets/CREDITS.md`.)
